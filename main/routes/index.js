...
connection.query('SELECT * FROM air_db.air', function(err, data, fields) { // 여기서 select한 field를 data에 저장
  if (err) throw err;
  console.log('데이터는?: ', data[0].humidity); // => humidity field의 배열 첫번째 값


router.get('/', function(req, res){
  
  console.log('http://localhost:5000/api'); 
  res.send({user :data}); // 위 주소에 json 형태로 파일 전송
