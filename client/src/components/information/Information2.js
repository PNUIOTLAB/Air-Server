...
export default function CustomizedTimeline2() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            axios.get('http://192.168.0.6:5000/room/102').then(res => {
                console.log(res);
                ...
                setGas(res.data.co2);
                if(D.getTime()-time1>=2000){
                    setDevice1(res.data.device1);
                    time1=0;
                } // device1 ? on : off 
                if(D.getTime()-time2>=2000){
                    setDevice2(res.data.device2);
                    time2=0;
                }
                if(D.getTime()-time3>=2000){
                    setDevice3(res.data.device3);
                    time3=0;
                }
                if(D.getTime()-time4>=2000){
                    setDevice4(res.data.device4);
                    time4=0;
                }
                if(D.getTime()-time5>=2000){
                    setDevice5(res.data.device5);
                    time5=0;
                }
                if(D.getTime()-time6>=2000){
    ...
    const onToggle1 = () => {
        ...
        axios.post('http://192.168.0.6:5000/devices',{
            '0': !device1,
            '1': null,
            '2': null,
            '3': null,
